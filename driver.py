...
from torchbenchmark.models import hf_GPT2, hf_T5, hf_GPT2_large, hf_Bert, hf_T5_large, timm_vision_transformer_large
...
model_names: List[str] = ['torchbenchmark.models.hf_Bert.Model', \
...
model_batch_sizes: Dict[str, int] = {'torchbenchmark.models.hf_Bert.Model': 32, \
...
class Experiment:
    ...
    def __init__(self, model_name:str, batch_size:int, extra_args=[]):
        ...
        model: BenchmarkModel = model_class("train", 'cuda', batch_size=batch_size, extra_args=extra_args)
        self.model = model.model
        self.model_type = type(model)
        self.batch_size = batch_size
        self.example_inputs = model.example_inputs

        if(self.model_type in ( hf_T5.Model, hf_GPT2.Model, hf_Bert.Model, hf_T5_large.Model, hf_GPT2_large.Model)):
            ...
            def timm_vit_train_step(model:nn.Module, optim:optim.Optimizer, example_inputs:Any):
...
def run_worker(rank, world_size):
    ...
    compiled_gm = make_fx(exp.train_step, tracing_mode='fake', _allow_non_fake_inputs=True)(exp.model, exp.optimizer, exp.example_inputs)
